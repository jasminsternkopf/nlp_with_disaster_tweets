...
def LSR_fit(X,y,no_of_latent_vars,return_vectors_for_predicting=False):
    E=X
    y2=-y.copy()
    y2=y2+1
    F=np.c_[y,y2]
...
def LSR_predict(X_test,xi,omega,principale_components,centroid,y_centroid):
    ...
    for k in range(no_of_latent_vars):
        ...
        e=e-np.outer(intermediate_var[:,k],principale_components[:,k]) #np.outer as we compute it for the whole matrix
    y_pred=np.c_[y_centroid[0]*np.ones(m_test), y_centroid[1]*np.ones(m_test)]+intermediate_var@omega.T
    return y_pred
...
class LSR_Estimator(BaseEstimator):
    ...
    def fit(self,X,y):
        self.xi,self.omega,self.principale_components,self.centroid,self.y_centroid=LSR_fit(X,y,self.no_of_latent_vars,return_vectors_for_predicting=True)
        return self
    
    def predict(self,X):
        ypred=LSR_predict(X,self.xi,self.omega,self.principale_components,self.centroid,self.y_centroid)
        y=[1 if ypred[i,0]>ypred[i,1] else 0 for i in range(ypred.shape[0])]
        return y

class LSR_Transformer(TransformerMixin,BaseEstimator): #brauche BaseEstimator damit get_params und set_params implementiert ist
