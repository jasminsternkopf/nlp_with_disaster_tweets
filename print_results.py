...
def print_test_train_params(test,train,params=None,dims=None):
    ...
    if params==None:
        ...
        for i,dim in enumerate(dims):
            print(dim,'       ',np.round(test[i],4),'     ',np.round(train[i],4),'   ',params[i])
        print(f'Bester Testscore: {test[best_index]} erreicht mit Dimension {dims[best_index]} und Parametern {params[best_index]}')